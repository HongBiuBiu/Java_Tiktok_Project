	...
	<view style="flex: 1;">
		...
		<list :pagingEnabled="true" :show-scrollbar="false" @scroll="listScroll" @scrollend="scroll" :scrollable="true" >	
			...
			<cell :recycle="false" 
				...
				<view class="publish-info-box">
					<view class="">
						...
						<view class="publish-info-music-box">
							...
							<text class="muisc-words">{{item.vlogerName}}original composition</text>
						</view>
					</view>
				...
				<!-- Display the operation buttons on the right side of the video, including avatar, like, comment, and share. -->
...
<script>
	...
	export default {
		...
		data() {
			return {
				thisVlog: {},			// Current short video object
				thisVlogId: "",			// Current short video primary key ID
				thisVlogerId: "",		// Primary key ID of the short video blogger
		...
		created() {
			...
			// Query short video list on the homepage.
		...
		methods: {
			// Adjust numbers exceeding 1000 or 10000, for example, 1.3k/6.8w.
			getGraceNumber(num) {
			...
			// Reset the list of liked videos.
			reLikePlayList(vlogId) {
				var me = this;
				var playerList = me.playerList;
				
				// After following, loop through the current player list and set the corresponding 'doIFollowVlogger' attribute of the fan relationship to true.
			...
			// Reset the list of disliked videos.
